...
확정된 도면(공간)의 크기와 용도에 맞게 실제 텍스트 및 시각적 콘텐츠를 다듬어 끼워 넣습니다.

- **담당 모듈:** LLM (텍스트 최적화) & CLIP (이미지 매칭)
- **입력:** Step 2에서 완성된 도면 JSON + 사용자의 원본 데이터 풀
- **동작:**
  - **[LLM 최적화]:** GNN이 할당해 준 `box`의 너비(`w`), 높이(`h`), 비율 등의 기하학적 제약을 인지합니다. 글이 영역을 벗어나 오버플로우가 발생하지 않도록 원본 텍스트 분량을 축소/요약하거나 확장합니다.
  - **[CLIP 매칭]:** `Picture` 또는 `Table` 노드 주변의 텍스트(문맥)를 분석하고, 사용자가 제공한 원시 이미지 풀(Pool)에서 가장 관련성이 높은(유사도 스코어 기반) 시각 자료를 선별해 맵핑합니다.
- **JSON 상태:** 좌표뿐만 아니라 내용(content)까지 모두 채워진 **최종 통합 데이터 소스(Single Source of Truth)**가 됩니다.
...
### Step 4. 렌더 (Frontend Rendering)

완성된 단일 데이터 소스(JSON)를 프론트엔드로 전달하여 실제 문서로 시각화합니다.

- **담당 모듈:** Next.js (프론트엔드) & FastAPI (백엔드)
- **입력:** Step 3의 최종 통합 JSON
- **동작:**
  - **오버플로우 방지:** JSON에 명시된 절대 좌표 스키마를 브라우저 렌더러가 그대로 해석하여 컴포넌트를 배치합니다.
  - **단일 소스 기반:** 본문 텍스트와 참조된 차트가 동일한 JSON을 공유하므로 수치 불일치(Hallucination)가 원천 차단됩니다.
- **출력:** 사용자가 화면 상에서 즉시 검토하고 수정(피드백)하거나 PDF/HTML 형식으로 추출할 수 있는 프로덕션 수준의 최종 학술 보고서가 출력됩니다.
